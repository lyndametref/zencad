...
class link(zencad.assemble.unit):
	def __init__(self, h=40, ax=(0,1,0)):
		...
		self.rotator = zencad.mbody.kinematic.rotator(parent=self, ax=ax, location=up(h))

r = zencad.mbody.kinematic.rotator(ax=(0,0,1))
a = link(ax=(0,1,0))
b = link(ax=(1,0,0))
c = link(ax=(0,1,0))
d = link(ax=(1,0,0))
...
chain = zencad.mbody.kinematic.kinematic_chain(LINKS[-1].rotator.output)
